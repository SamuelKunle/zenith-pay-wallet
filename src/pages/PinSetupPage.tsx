import { ArrowLeft, Lock, Fingerprint, CheckCircle2, Shield, Sparkles } from "lucide-react";
...
const Numpad = ({ onInput, onDelete }: { onInput: (v: string) => void; onDelete: () => void }) => (
...
const PinSetupPage = () => {
  ...
  const [step, setStep] = useState<"create" | "confirm" | "biometric" | "success">("create");
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 overflow-x-hidden">
        ...
        >
          ...
          <h1 className="text-[20px] font-bold text-foreground mb-1">You're all set!</h1>
          <p className="text-[13px] text-muted-foreground max-w-xs leading-relaxed mb-8">
            Your wallet is ready. Start sending, receiving, and managing your money.
          </p>
          <div className="w-full space-y-2.5 mb-8">
            {[
              ...
              return (
                <div key={item.label} className="surface-interactive flex items-center gap-3 px-4 py-3.5">
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col overflow-x-hidden">
        <header className="flex items-center gap-3 px-5 py-3">
          <button onClick={() => setStep("confirm")} className="flex h-9 w-9 items-center justify-center rounded-xl bg-secondary/60">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
          <motion.div {...fadeInUp}>
            ...
            <h1 className="text-[20px] font-bold text-foreground mb-2">Enable Biometrics?</h1>
            <p className="text-[13px] text-muted-foreground max-w-xs mx-auto leading-relaxed mb-10">
              Use fingerprint or Face ID to unlock your wallet and approve transactions instantly.
            </p>
          </motion.div>
          <div className="w-full max-w-xs space-y-3">
            ...
            <button onClick={() => setStep("success")} className="btn-secondary">
              Skip for now
            </button>
          </div>
        </div>
      ...
      <header className="flex items-center gap-3 px-5 py-3">
        ...
        <div className="flex-1">
          <div className="flex gap-1">
            ...
            <div className="h-1 flex-1 rounded-full bg-primary" />
            <div className={`h-1 flex-1 rounded-full ${step === "confirm" ? "bg-primary/40" : "bg-primary/40"}`} />
          </div>
        </div>
      </header>
